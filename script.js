...
const config = {
  ...
  /* Colocar img Base 64* padrão para error*/
...
function createPhotoCardElement(photo) {
  ...
  card.innerHTML = `
           ...
           <div class="photo-info">
               ...
               <button class="btn-delete" data_id="${photo._id}">Excluir</button>
           </div>
           `;

  
// // Seleciona o botão de deletar dentro do card
const dellButton = card.querySelector(".btn-delete");
 
// Adiciona clique ao botão de deletar
dellButton.addEventListener("click", async () => {
  // Chama a função deletePhoto passando o ID da foto
...
async function deletePhoto(id) {
  try {
    ...
    // recarrega as fotos atualizadas
    loadAndDisplayPhotos();
  } catch (error) {
